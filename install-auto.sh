...
# OpenShift 4 on Google Cloud Installation Script - Automated Version
# Kopie von install.sh fuer nicht-interaktiven Betrieb (keine Credentials enthalten)
...
# Logging
# Separate Log-Datei pro Lauf, damit alte Erfolgs-/Fehlermeldungen aus
# frueheren (ggf. laengst zerstoerten) Installationen nicht mit dem
# aktuellen Lauf verwechselt werden koennen.
LOG_FILE="openshift-install-$(date +%Y%m%d-%H%M%S).log"
exec > >(tee -a "$LOG_FILE")
exec 2>&1
...
wait_for_user() {
    echo -e "${YELLOW}$1${NC}"
    echo -e "${YELLOW}[Automatischer Modus: Fahre automatisch fort...]${NC}"
}
...
auto_install_missing_tools() {
    ...
    echo ""
    auto_install="j"
    echo "[Automatisch: Tools werden installiert]"
...
if check_quotas_automatically; then
    ...
    continue_anyway="j"
    echo "[Automatisch: Fahre trotz Quota-Warnung fort]"
...
if [ -f "$DEFAULT_JSON_PATH" ]; then
    ...
    if validate_service_account_json "$DEFAULT_JSON_PATH"; then
        ...
        echo ""
        json_option=1
        echo "[Automatisch: Verwende vorhandene JSON Datei (Option 1)]"
...
if monitor_dns_propagation "$GCP_DOMAIN"; then
    ...
    echo ""
    continue_dns="j"
    echo "[Automatisch: Fahre mit Installation trotz DNS-Warnung fort]"
...
if ! auto_detect_ssh_key; then
    generate_key="j"
    echo "[Automatisch: Generiere neuen SSH Key]"
...
# Erstelle install-config.yaml fuer nicht-interaktiven Betrieb
print_step "Erstelle install-config.yaml..."
PULL_SECRET_JSON=$(jq -c . "../pull-secret.txt")
SSH_KEY_CONTENT=$(cat "$SSH_KEY_PATH")

{
    echo "apiVersion: v1"
    echo "baseDomain: $GCP_DOMAIN"
    echo "metadata:"
    echo "  name: training"
    echo "compute:"
    ...
    echo "platform:"
    echo "  gcp:"
    echo "    projectID: $GCP_PROJECT"
    echo "    region: $GCP_REGION"
    echo "credentialsMode: Mint"
    printf "pullSecret: '%s'\n" "$PULL_SECRET_JSON"
    echo "sshKey: |"
    echo "  $SSH_KEY_CONTENT"
} > install-config.yaml
print_step "install-config.yaml erstellt ✓"
